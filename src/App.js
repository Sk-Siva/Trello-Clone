...
const App = () => {
  const [boards, setBoards] = useState([
    { id: 1, title: "Dummy 1", items: [{ id: 1, content: "Item 1" }, { id: 2, content: "Item 2" }] },
    { id: 2, title: "Dummy 2", items: [{ id: 3, content: "Item 1" }, { id: 4, content: "Item 2" }] }
  ]);
  ...
  const addItem = (boardId, newItemContent) => {
    const updatedBoards = boards.map(board => 
      board.id === boardId ? { ...board, items: [...board.items, { id: Date.now(), content: newItemContent }] } : board
    );
    updateBoards(updatedBoards);
  };

  const editBoardTitle = (boardId, newTitle) => {
    updateBoards(boards.map(board => board.id === boardId ? { ...board, title: newTitle } : board));
  };

  const editItemContent = (boardId, itemId, newContent) => {
    updateBoards(boards.map(board => 
      board.id === boardId
        ? { ...board, items: board.items.map(item => item.id === itemId ? { ...item, content: newContent } : item) }
        : board
    ));
  };

  const deleteBoard = (boardId) => updateBoards(boards.filter(board => board.id !== boardId));
  
  const deleteItem = (boardId, itemId) => updateBoards(boards.map(board => 
    ...
    if (dragItem && dragBoard && dragBoard !== targetBoardId) {
      ...
      const updatedBoards = boards.map(board => {
        if (board.id === dragBoard) {
          return { ...board, items: board.items.filter(i => i.id !== dragItem) };
        }
  ...
  const handleAddBoard = () => {
    if (newBoardTitle.trim()) {
      const newBoard = { id: nextBoardId.current++, title: newBoardTitle.trim(), items: [] };
